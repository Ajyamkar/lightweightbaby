      ...
      axios
        ...
        .then((res) => {
          ...
          if (res.data.code === "500") {
            this.setState({
              error: { ...this.state.error, authenication: res.data.err },
            });
          }
    ...
    return (
      <div className="Join">
        <div className="Join-main-div">
          <h1 style={{ paddingTop: "15px" }} className="Join-h1">
            Join
          </h1>
          <div
            onClick={() => {
          ...
          <div
            onClick={() => {
              window.location.href = "http://localhost:9000/auth/fbAuth";
            }}
            className="social-Join-btn"
          >
            {" "}
          ...
          <form onSubmit={this.handleSubmit}>
            <div>
              ...
              <TextField
                ...
                required
              />
              {this.state.showPasswordConditions ? (
                <div>
                  <p id="lower" className="password-errors">
                    *Should contain atleast one lowercase letter
                  </p>
                  <p id="upper" className="password-errors">
                    *Should contain atleast one uppercase letter
                  </p>
                  <p id="number" className="password-errors">
                    *Should contain atleast one numeric digit
                  </p>
                  <p id="special" className="password-errors">
                    *Should contain atleast one special character eg @,# etc
                  </p>
                  <p id="length" className="password-errors">
              ...
              <TextField
                ...
                InputProps={{
                  startAdornment: (
                    <InputAdornment position="start">
                      {this.state.showPassword ? (
                        <LockOpenIcon className={{ root: "form-icon" }} />
                      ) : (
                        <LockOutlinedIcon className={{ root: "form-icon" }} />
                      )}
                    </InputAdornment>
                  ),
                  endAdornment: (
            ...
            {this.state.error.authenication === "" ? (
              <Button
                id="join-submit-btn"
                type="submit"
                classes={{ root: "Join-submit-btn" }}
                style={{ margin: "5px 25vw 0" }}
              >
                Join
              </Button>
            ) : (
              <p className="authentication-error">
                {this.state.error.authenication} try to login
              </p>
            )}
          </form>
          <p className="login-p">
