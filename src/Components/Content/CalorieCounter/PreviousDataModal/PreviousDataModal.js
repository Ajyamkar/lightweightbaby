...
import {
  sumOfMacronutrients,
  sumOfMicroNutrients,
} from "../getSumOfNutrientsFunction";
import {
  totalMacroNutritionCol,
  totalMicroNutritionCol,
} from "../Lists/TotalNutritionColumnList";
...
export default class PreviousDataModal extends Component {
  ...
  handleDateChange(date) {
    ...
    api
      .post("/saveMealsData/getPreviousDataForSelectedDate", {
        token: token,
        selectedDate: date,
      })
      .then((result) => {
        if (result.data.length !== 0) {
          const breakfastNutritionData = {
            microNutrients: sumOfMicroNutrients(
              result.data[0].allMealsData.Breakfast
            ),
            macroNutrients: sumOfMacronutrients(
              result.data[0].allMealsData.Breakfast
            ),
          };
          ...
          const morningSnackNutritionData = {
            microNutrients: sumOfMicroNutrients(
              result.data[0].allMealsData.MorningSnack
            ),
            macroNutrients: sumOfMacronutrients(
              result.data[0].allMealsData.MorningSnack
            ),
          ...
          const lunchNutritionData = {
            microNutrients: sumOfMicroNutrients(
              result.data[0].allMealsData.Lunch
            ),
            macroNutrients: sumOfMacronutrients(
              result.data[0].allMealsData.Lunch
            ),
          ...
          const eveningsnackNutritionData = {
            microNutrients: sumOfMicroNutrients(
              result.data[0].allMealsData.EveningSnack
            ),
            macroNutrients: sumOfMacronutrients(
              result.data[0].allMealsData.EveningSnack
            ),
          ...
          const dinnerNutritionData = {
            microNutrients: sumOfMicroNutrients(
              result.data[0].allMealsData.Dinner
            ),
            macroNutrients: sumOfMacronutrients(
              result.data[0].allMealsData.Dinner
            ),
          ...
          const totalAnaylsisOfMacrosArr = [
            breakfastNutritionData.macroNutrients === 0
              ? zeroMacros
              : breakfastNutritionData.macroNutrients,
            morningSnackNutritionData.macroNutrients === 0
              ? zeroMacros
              : morningSnackNutritionData.macroNutrients,
            lunchNutritionData.macroNutrients === 0
              ? zeroMacros
              : lunchNutritionData.macroNutrients,
            eveningsnackNutritionData.macroNutrients === 0
              ? zeroMacros
              : eveningsnackNutritionData.macroNutrients,
            dinnerNutritionData.macroNutrients === 0
              ? zeroMacros
              : dinnerNutritionData.macroNutrients,
          ];

          const totalAnaylsisOfMicrosArr = [
            breakfastNutritionData.microNutrients === 0
              ? zeroMicros
              : breakfastNutritionData.microNutrients,
            morningSnackNutritionData.microNutrients === 0
              ? zeroMicros
              : morningSnackNutritionData.microNutrients,
            lunchNutritionData.microNutrients === 0
              ? zeroMicros
              : lunchNutritionData.microNutrients,
            eveningsnackNutritionData.microNutrients === 0
              ? zeroMicros
              : eveningsnackNutritionData.microNutrients,
            dinnerNutritionData.microNutrients === 0
              ? zeroMicros
              : dinnerNutritionData.microNutrients,
          ];
  ...
  showPast7DaysData() {
    ...
    if (this.state.showPrevious7DaysDataBarChart) {
      ...
      api
        .post("/saveMealsData/getLast7DaysData", { token: token })
        .then((result) => {
  ...
  render() {
    ...
    this.state.past7DaysPreviousData.map((data) => {
      previous7DaysDatelabels.push(
        data.date.toString().slice(8, 10) +
          "-" +
          data.date.toString().slice(5, 7) +
          "-" +
            ...
            <div style={paperStyle} classes={"paper"}>
              ...
              <div className="paper-content-div">
                ...
                <div className="showPrevious7Days-btn-div">
                  <Button
                    className="showPrevious7Days-btn"
                    onClick={this.showPast7DaysData}
                  >
                    {this.state.showPrevious7DaysDataBarChart ? "Hide" : "Show"}{" "}
                    Past 7 days data
                  </Button>
                </div>
                ...
                {this.state.dataFound ? (
                  <div className="data-found-div">
                    {mealArr.map((meal, index) => {
                      return (
                        <div
                          key={index}
                          className={"previousData-table-main-div"}
                        >
                          ...
                          {this.state.totalNutrientsForAMeals[meal]
                            .macroNutrients === 0 ? null : (
                            <PieChart
                              totalCalories={
                                this.state.totalNutrientsForAMeals[meal]
                                  .macroNutrients.totalCalories
                              }
                    ...
                    <h1 style={{ textAlign: "center" }}>
                      Full anaylsis Of Nutritents
                    </h1>
                    <div style={{ margin: "2vh 0" }}>
                      <h2 className={"macro-full-anaylsis-heading"}>
                        Macro Nutritents
                      </h2>
                      <DataGrid
                        rows={[
                          {
                            id: 1,
                            protein:
                              this.state.totalNutrientsAnaylsisOfWholeDay
                                .macroNutrients.protein,
                            carbs:
                              this.state.totalNutrientsAnaylsisOfWholeDay
                                .macroNutrients.carbs,
                            fats: this.state.totalNutrientsAnaylsisOfWholeDay
                              .macroNutrients.fats,
                            sugar:
                              this.state.totalNutrientsAnaylsisOfWholeDay
                                .macroNutrients.sugar,
                        ...
                        rows={[
                          {
                            ...
                            calcium:
                              this.state.totalNutrientsAnaylsisOfWholeDay
                                .microNutrients.calcium,
                            iron: this.state.totalNutrientsAnaylsisOfWholeDay
                              .microNutrients.iron,
                            zinc: this.state.totalNutrientsAnaylsisOfWholeDay
                              .microNutrients.zinc,
                            folate:
                              this.state.totalNutrientsAnaylsisOfWholeDay
                                .microNutrients.folate,
                  ...
                  <p className="dataNotFound-p">
                    Data not found for above date
                  </p>
                ) : null}
              </div>
            </div>
